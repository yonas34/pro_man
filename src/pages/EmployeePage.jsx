...
const setDataFromDialogue=async(datas,img)=>{
  ...
  .then(async(response) =>{
await axios.post("https://www.nrwlpms.com/api/api/update_emp_pic.php", {
  "emp_id" : datas.emp_id,
  "emp_pic" : img,
  "jwt" : user.token
}).then((response)=>{
...
setData([...dataUpdate]);
  ...
  <div>
    ...
    <Dialogue open={open}   manData={manPowerData} setData={(data,img)=>setDataFromDialogue(data,img)} data={selected} onClose={handleDetailClose} onSave={handleDetailSave}/>
    </div>
  );
}
